...
const PersonnelAcademiqueProvincePage: React.FC = () => {
  ...
  // Filtrer uniquement le personnel académique de cette province
  const personnelsAcademiques = useMemo(() => {
    return personnelsProvince.filter(p => p.type === 'Académique');
  }, [personnelsProvince]);
  ...
  const stats = useMemo(() => {
    const total = personnelsAcademiques.length;
    const actifs = personnelsAcademiques.filter(p => p.statut === 'Actif').length;
    const salaireMoyen = total > 0 
      ? personnelsAcademiques.reduce((sum, p) => sum + p.salaire, 0) / total 
      : 0;
    
    const grades = personnelsAcademiques.reduce((acc, p) => {
      if (p.grade) {
    ...
    return { total, actifs, salaireMoyen, grades };
  }, [personnelsAcademiques]);
  ...
  return (
    <div className="p-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
              Personnel Académique - {province.designation}
            </h1>
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
              Gestion du personnel enseignant et académique
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        />
        <StatCard
          title="Grades Différents"
          value={Object.keys(stats.grades).length}
          icon="🎓"
          color="purple"
        />
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm">
        <div className="p-6 border-b border-gray-200 dark:border-gray-700">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
            Liste du Personnel Académique
          </h3>
        </div>
        <div className="p-6">
          {personnelsAcademiques.length === 0 ? (
            <div className="text-center py-12">
              <div className="text-gray-400 text-6xl mb-4">👨‍🏫</div>
              <h3 className="text-lg font-medium text-gray-700 dark:text-gray-300 mb-2">
                Aucun personnel académique
              </h3>
              <p className="text-sm text-gray-500 dark:text-gray-400">
                Il n'y a pas encore de personnel académique dans cette province.
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {personnelsAcademiques.map((personnel) => (
