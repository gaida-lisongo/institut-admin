...
const SystemeEditCard: React.FC<SystemeEditCardProps> = ({ systeme, onSuccess, onCancel }) => {
    const { updateSysteme, loading } = useSystemeStore();
    ...
    const [formData, setFormData] = useState<SystemeFormData>({
        ...
        cycles: systeme.cycles.map(cycle => ({
            ...cycle,
    ...
    // Étape 1: Modification du système
    const renderStep1 = () => (
        <div className="space-y-6">
            <div className="text-center mb-6">
                ...
                <p className="text-gray-600 dark:text-gray-400">
                    Modifiez les informations de base de votre système éducatif
                </p>
            </div>
    ...
    // Étape 2: Modification des cycles
    const renderStep2 = () => (
        <div className="space-y-6">
            <div className="text-center mb-6">
                <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">
                ...
                <p className="text-gray-600 dark:text-gray-400">
                    Modifiez les cycles qui composent votre système éducatif
                </p>
            </div>
                ...
                <div key={cycleIndex} className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                    <div className="flex items-center justify-between mb-4">
                        <h4 className="font-medium text-gray-900 dark:text-white">
                            {cycle.designation || `Cycle ${cycleIndex + 1}`}
                        </h4>
                        <button
                            onClick={() => {
    ...
    // Étape 3: Modification des classes
    const renderStep3 = () => (
        <div className="space-y-6">
            <div className="text-center mb-6">
                <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">
                ...
                <p className="text-gray-600 dark:text-gray-400">
                    Modifiez les classes pour chaque cycle
                </p>
            </div>
                        ...
                        <div key={classeIndex} className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-3 mb-3">
                            <div className="flex items-center justify-between mb-3">
                                <span className="text-sm font-medium text-gray-700 dark:text-gray-300">
                                    {classe.niveau || `Classe ${classeIndex + 1}`}
                                </span>
                                <button
                                    onClick={() => {
                                        const newCycles = [...formData.cycles];
                            ...
                            <div className="space-y-2">
                                ...
                                {classe.description.map((desc, descIndex) => (
                                    <div key={descIndex} className="flex items-center space-x-2">
                                        <input
                                            ...
                                            onChange={(e) => {
                                                const newCycles = [...formData.cycles];
                                                newCycles[cycleIndex].classes[classeIndex].description[descIndex] = e.target.value;
                                                setFormData({...formData, cycles: newCycles});
                                            }}
                                            placeholder="Description de la classe..."
                                            className="flex-1 px-2 py-1 text-sm border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-1 focus:ring-blue-500 focus:border-transparent"
                                        />
                                        {classe.description.length > 1 && (
                                            <button
                                                onClick={() => {
                                                    const newCycles = [...formData.cycles];
                                                    newCycles[cycleIndex].classes[classeIndex].description = classe.description.filter((_, i) => i !== descIndex);
                                                    setFormData({...formData, cycles: newCycles});
                                                }}
                                ...
                                <button
                                    onClick={() => {
                                        const newCycles = [...formData.cycles];
                                        newCycles[cycleIndex].classes[classeIndex].description.push('');
                                        setFormData({...formData, cycles: newCycles});
                                    }}
    ...
    const handleSubmit = async () => {
        ...
        try {
            const result = await updateSysteme(systeme._id, formData);
            if (result) {
                onSuccess();
            }
        } catch (error) {
            ...
            <div className="bg-gray-50 dark:bg-gray-700/50 px-6 py-4">
                <div className="flex items-center justify-between">
                    <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
                        Modifier le système: {systeme.designation}
                    </h2>
...
export default SystemeEditCard;
