...
import { Edit2, Trash2, Book, Users, GraduationCap, Plus, Settings } from 'lucide-react';
import { Cycle } from '@/services/CycleService';
import { useCycleStore } from '@/stores/cycleStore';
import ClasseSemestresModal from './ClasseSemestresModal';
...
const CycleList: React.FC<CycleListProps> = ({ cycles, loading, onEdit, sectionId }) => {
  const { deleteCycle, fetchCyclesBySection } = useCycleStore();
  ...
  const handleClasseClick = (classe: any, cycleId: string, classeIndex: number) => {
    setSelectedClasse({ classe, cycleId, classeIndex });
  };

  const handleClasseUpdated = async () => {
    // Rafraîchir les cycles pour voir les changements
    await fetchCyclesBySection(sectionId);
  };
  ...
  return (
    <>
              ...
              {cycle.classes.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {cycle.classes.map((classe, classeIndex) => (
                    <div
                      key={classeIndex}
                      className="border border-gray-200 dark:border-gray-600 rounded-lg p-4 hover:shadow-md transition-shadow cursor-pointer bg-gray-50 dark:bg-gray-700/50"
                      onClick={() => handleClasseClick(classe, cycle._id!, classeIndex)}
                    >
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            handleClasseClick(classe, cycle._id!, classeIndex);
                          }}
                      ...
                      {classe.semestres.length > 0 && (
                        <div className="mt-2 flex flex-wrap gap-1">
                          {classe.semestres.slice(0, 3).map((_, index) => (
                            <div
                              key={index}
      ...
      {/* Modal des semestres */}
      {selectedClasse && (
        <ClasseSemestresModal
          isOpen={!!selectedClasse}
          onClose={() => setSelectedClasse(null)}
          classe={selectedClasse.classe}
          cycleId={selectedClasse.cycleId}
          classeIndex={selectedClasse.classeIndex}
          onClasseUpdated={handleClasseUpdated}
        />
      )}
    </>
  );
